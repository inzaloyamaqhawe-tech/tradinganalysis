...
## How it works
- Signal engine (`signals.js`, ported from `BOTS/universal.py`): ATR-relative regime classification (ranging/trending/breakout/quiet) with 5 regime-matched strategies (CRT, trend-pullback, breakout, mean-reversion, classic chart pattern), TP1-4/SL ladder at a disciplined 1:2 risk:reward. Plus a 6th, higher-priority strategy (`smc.js`) — Smart Money Concepts: 4H market structure for directional bias, an unmitigated 1H order block/fair value gap as the key zone, and a 15min liquidity-sweep-and-reclaim as entry confirmation. Crypto only (see `smc.js`'s integration note in `server.js` for why FX/gold are excluded — Twelve Data's free-tier daily quota).
